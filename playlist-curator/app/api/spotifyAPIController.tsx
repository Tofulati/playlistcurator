...
export class SpotifyAPIController {
    ...
    async getAccessToken(){
        ...
        var res = await fetch('https://accounts.spotify.com/api/token',authParams)
            .then(res => res.json())
            .then(data => {this.accessToken = data.access_token})
    ...
    async getSong(trackEndPoint) {
        ...
        var result = await fetch(`https://api.spotify.com/v1/tracks/${trackEndPoint}`, {
            method: 'GET',
    ...
    async getArtist(artistEndPoint){
        await this.getAccessToken();
        const result = await fetch(`https://api.spotify.com/v1/artists/${artistEndPoint}`, {
            method: 'GET',
            headers: { 'Authorization' : 'Bearer ' + this.accessToken}
        })
            .then(res => res.json())
    ...
    async getPlaylist(playlistEndPoint){
        await this.getAccessToken();
        const result = await fetch(`https://api.spotify.com/v1/playlists/${playlistEndPoint}`, {
            method: 'GET',
            headers: { 'Authorization' : 'Bearer ' + this.accessToken}
        })
            .then(res => res.json())
    ...
    async getPlaylistTrackArtists(playlistEndPoint){
        ...
        const result = await fetch(`https://api.spotify.com/v1/playlists/${playlistEndPoint}`, {
            ...
            .then(data => {
                for(let i = 0; i < data.tracks.limit; i++){
                    this.playlistTrackArtists[i] = data.tracks.items[i].track.artists[0].name;
                    //console.log(data.tracks.items[i].track.name)
                }
             })
    }
    ...
    async getPlaylistTrackCovers(playlistEndPoint){
        ...
        const result = await fetch(`https://api.spotify.com/v1/playlists/${playlistEndPoint}`, {
            ...
            .then(data => {
                for(let i = 0; i < data.tracks.limit; i++){
                    this.playlistTrackCovers[i] = data.tracks.items[i].track.album.images[0].url;
